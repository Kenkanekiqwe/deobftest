...
impl Profile {
    ...
    fn engine_name(self) -> &'static str { match self { Self::Safe => "safe", Self::Balanced => "balanced", Self::Maximum => "maximum" } }
}
...
#[derive(Debug, Clone)]
enum Message { PickInput, PickOutput, InputChanged(String), OutputChanged(String), PasswordChanged(String), ProfileChanged(Profile), Analyze, Protect, Restore, Verify }
...
impl App {
    ...
    fn analyze(&mut self) {
        ...
        match std::fs::read(&path) {
            Ok(data) => match analyze_only(&data) {
                Ok(a) => {
                    self.analysis = Some(format!("Type: {}\nArchitecture: {}\nExecutable: {}\nDebug markers: {}\nArchive signature: {}\nSize: {} bytes", a.kind, a.architecture, a.executable, a.has_debug_markers, a.has_archive_signature, data.len()));
    ...
    fn view(&self) -> Element<'_, Message> {
        ...
        let profile = self.profile.unwrap_or(Profile::Balanced);
        let header = row![column![text("DEOBF").size(38), text("Windows Protection Studio").size(16)].spacing(3), container(text(if self.busy { "WORKING" } else { "READY" }).size(13)).padding(8)].spacing(24).align_y(iced::Alignment::Center);
        let input = row![text_input("Input file / .deobf package", &self.input).on_input(Message::InputChanged).width(Length::Fill), button(text("Browse")).on_press(Message::PickInput)].spacing(10);
        ...
        let security = row![column![text("Protection profile"), pick_list(&Profile::ALL[..], Some(profile), Message::ProfileChanged).width(Length::Fill)].spacing(7).width(Length::Fill), column![text("Password"), text_input("Minimum 12 characters", &self.password).secure(true).on_input(Message::PasswordChanged)].spacing(7).width(Length::Fill)].spacing(20);
        let actions = row![
