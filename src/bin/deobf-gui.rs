...
enum Message {
    ...
    ProfileChanged(Profile), Analyze, Protect, Verify,
}
...
impl App {
    ...
    fn protect(&mut self) {
        ...
        match result {
            Ok(report) => self.status = format!(
                "Encrypted package created: {} → {} bytes, {} pass(es), integrity enabled.",
    ...
    fn verify(&mut self) {
        if self.password.len() < 12 { self.status = "Password must contain at least 12 characters.".into(); return; }
        if self.input.is_empty() { self.status = "Choose a protected compatibility file.".into(); return; }
        self.busy = true;
        match verify_compatible(&PathBuf::from(&self.input), self.password.as_bytes()) {
            Ok(manifest) => self.status = format!("Legacy metadata OK — {} bytes, {} profile, hash verified.", manifest.original_size, manifest.profile),
            Err(error) => self.status = format!("Legacy metadata verification failed: {error:#}"),
        }
        self.busy = false;
    }
    ...
    fn view(&self) -> Element<'_, Message> {
        ...
        let input = row![
            text_input("Input file", &self.input).on_input(Message::InputChanged).width(Length::Fill),
            button(text("Browse")).on_press(Message::PickInput)
        ].spacing(10);
        let output = row![
            text_input("Output package (.deobf)", &self.output).on_input(Message::OutputChanged).width(Length::Fill),
        ...
        let actions = row![
            ...
            button(text("Verify legacy metadata")).on_press_maybe((!self.input.is_empty() && !self.busy).then_some(Message::Verify))
        ].spacing(10);
