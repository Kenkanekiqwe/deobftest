...
fn read_all_limited(input: &Path) -> Result<Vec<u8>> {
    ...
    if len > (usize::MAX as u64) { bail!("file is too large for this operation"); }
...
fn protect(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    ...
    loop {
        ...
        let encrypted = cipher.encrypt(
            &nonce(&base, index),
            chacha20poly1305::aead::Payload { msg: &compressed, aad: &aad(index, plain_len, 1) }
        ...
        let mut p = [0u8; 2]; OsRng.fill_bytes(&mut p);
    ...
    let trailer = cipher.encrypt(
        &nonce(&base, u64::MAX),
        chacha20poly1305::aead::Payload { msg: final_hash.as_bytes(), aad: &aad(u64::MAX, plain_len, 1) }
...
fn read_header(src: &mut File) -> Result<(u8, u8, [u8; SALT_LEN], [u8; NONCE_LEN], u64)> {
    ...
    let mut flags = [0u8; 1];
    if ver[0] == VERSION { src.read_exact(&mut flags)?; }
    let mut salt = [0u8; SALT_LEN];
    let mut base = [0u8; NONCE_LEN];
    let mut len = [0u8; 8];
...
fn unprotect_v1(src: &mut File, output: &Path, pass: &[u8], salt: &[u8; SALT_LEN], base: &[u8; NONCE_LEN], plain_len: u64) -> Result<()> {
    ...
    let mut total = 0u64; let mut index = 0u64;
    loop {
        let mut p = [0u8; 4];
        ...
        src.read_exact(&mut p)?; let enc_n = u32::from_le_bytes(p) as usize;
...
fn unprotect(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    ...
    if version == LEGACY_VERSION { return unprotect_v1(&mut src, output, pass, &salt, &base, plain_len); }
    ...
    let mut dst = File::create(&tmp)?;
    let mut total = 0u64; let mut index = 0u64;
    let mut digest = Hasher::new(); digest.update(b"DEOBF-CONTENT-V2");

    loop {
        let mut marker = [0u8; 4];
        src.read_exact(&mut marker)?;
        if &marker == b"TRLR" {
            let mut l = [0u8; 4]; src.read_exact(&mut l)?;
            ...
            let expected = cipher.decrypt(&nonce(&base, u64::MAX), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad(u64::MAX, plain_len, flags) }).map_err(|_| anyhow::anyhow!("trailer authentication failed"))?;
        ...
        let mut e = [0u8; 4]; src.read_exact(&mut e)?;
        let enc_n = u32::from_le_bytes(e) as usize;
        if n == 0 || n > CHUNK || enc_n < TAG_LEN || enc_n > CHUNK + TAG_LEN { let _ = fs::remove_file(&tmp); bail!("invalid container chunk"); }
        let mut enc = vec![0u8; enc_n]; src.read_exact(&mut enc)?;
        let mut p = [0u8; 2]; src.read_exact(&mut p)?;
        ...
        digest.update(&plain); dst.write_all(&plain)?;
        total += n as u64; index += 1;
    }
    if total != plain_len { let _ = fs::remove_file(&tmp); bail!("length mismatch: container is damaged"); }
    dst.sync_all()?; drop(dst); fs::rename(tmp, output)?; Ok(())
}
...
fn debugger_present() -> bool {
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(s) = fs::read_to_string("/proc/self/status") {
            return s.lines().any(|l| l.trim_start().starts_with("TracerPid:") && l.split_whitespace().nth(1).unwrap_or("0") != "0");
        }
        false
    }
...
fn run_jar(input: &Path, java: &str, pass: &[u8], args: &[String]) -> Result<()> {
    if debugger_present() { bail!("runtime hardening: debugger detected"); }
    ...
    let status = status?;
    if !status.success() { bail!("java exited with {}", status); }
    Ok(())
}

fn tempfile_dir() -> Result<PathBuf> {
    let mut r = [0u8; 16]; OsRng.fill_bytes(&mut r);
