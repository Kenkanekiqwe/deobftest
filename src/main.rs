...
fn protect(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    ...
    let tmp = output.with_extension("deobf-write-tmp"); let mut dst = File::create(&tmp).with_context(|| format!("create {}", tmp.display()))?;
    ...
    loop {
        ...
        let mut p = [0u8; 2]; OsRng.fill_bytes(&mut p); let pad = (u16::from_le_bytes(p) as usize) % (MAX_PAD + 1); dst.write_all(&(pad as u16).to_le_bytes())?;
        if pad != 0 { let mut padding = vec![0u8; pad]; OsRng.fill_bytes(&mut padding); dst.write_all(&padding)?; } index += 1;
    }
    let final_hash = digest.finalize(); let trailer = cipher.encrypt(&nonce(&base, u64::MAX), chacha20poly1305::aead::Payload { msg: final_hash.as_bytes(), aad: &aad(u64::MAX, plain_len, 1) }).map_err(|_| anyhow::anyhow!("trailer encryption failed"))?;
...
fn read_header(src: &mut File) -> Result<Header> {
    let mut magic = [0u8; 8]; src.read_exact(&mut magic)?;
    if &magic != MAGIC { bail!("not a DEOBF container"); }
    let mut ver = [0u8; 1]; src.read_exact(&mut ver)?;
    if ver[0] != VERSION && ver[0] != LEGACY_VERSION { bail!("unsupported container version {}", ver[0]); }
    let mut flags = [0u8; 1]; if ver[0] == VERSION { src.read_exact(&mut flags)?; }
    let mut salt = [0u8; SALT_LEN]; let mut base = [0u8; NONCE_LEN]; let mut len = [0u8; 8];
    src.read_exact(&mut salt)?; src.read_exact(&mut base)?; src.read_exact(&mut len)?;
    Ok(Header { version: ver[0], flags: flags[0], salt, base, plain_len: u64::from_le_bytes(len) })
}

fn unprotect_v1(src: &mut File, output: &Path, pass: &[u8], salt: &[u8; SALT_LEN], base: &[u8; NONCE_LEN], plain_len: u64) -> Result<()> {
    let key = derive_key(pass, salt)?; let cipher = XChaCha20Poly1305::new((&key).into()); let tmp = output.with_extension("deobf-tmp");
    let mut dst = File::create(&tmp)?; let mut total = 0u64; let mut index = 0u64;
    loop {
        ...
        if n == 0 || n > CHUNK || enc_n != n + TAG_LEN { let _ = fs::remove_file(&tmp); bail!("invalid container chunk"); }
        let mut enc = vec![0u8; enc_n]; src.read_exact(&mut enc)?;
        let plain = cipher.decrypt(&nonce_v1(base, index), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad_v1(index, plain_len) }).map_err(|_| anyhow::anyhow!("authentication failed: wrong password or modified file"))?;
...
fn nonce_v1(base: &[u8; NONCE_LEN], index: u64) -> XNonce {
    let mut h = Hasher::new(); h.update(base); h.update(&index.to_le_bytes()); let digest = h.finalize(); let mut n = [0u8; NONCE_LEN]; n.copy_from_slice(&digest.as_bytes()[..NONCE_LEN]); XNonce::from(n)
}

fn aad_v1(index: u64, plain_len: u64) -> Vec<u8> {
    let mut a = Vec::with_capacity(25); a.extend_from_slice(MAGIC); a.push(LEGACY_VERSION); a.extend_from_slice(&index.to_le_bytes()); a.extend_from_slice(&plain_len.to_le_bytes()); a
}

fn unprotect(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    if input == output { bail!("input and output must differ"); }
    let mut src = File::open(input)?; let header = read_header(&mut src)?;
    if header.version == LEGACY_VERSION { return unprotect_v1(&mut src, output, pass, &header.salt, &header.base, header.plain_len); }
    if header.flags & 1 == 0 { bail!("unsupported DEOBF v2 flags"); }
    let key = derive_key(pass, &header.salt)?; let cipher = XChaCha20Poly1305::new((&key).into()); let tmp = output.with_extension("deobf-tmp");
    ...
    loop {
        ...
        if &marker == b"TRLR" {
            let mut l = [0u8; 4]; src.read_exact(&mut l)?; let n = u32::from_le_bytes(l) as usize;
            if n != 48 { let _ = fs::remove_file(&tmp); bail!("invalid trailer"); }
            let mut enc = vec![0u8; n]; src.read_exact(&mut enc)?;
            let expected = cipher.decrypt(&nonce(&header.base, u64::MAX), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad(u64::MAX, header.plain_len, header.flags) }).map_err(|_| anyhow::anyhow!("trailer authentication failed"))?;
            if expected.len() != 32 || expected.as_slice() != digest.finalize().as_bytes() { let _ = fs::remove_file(&tmp); bail!("content integrity check failed"); }
            break;
        }
        let n = u32::from_le_bytes(marker) as usize; let mut e = [0u8; 4]; src.read_exact(&mut e)?; let enc_n = u32::from_le_bytes(e) as usize;
        if n == 0 || n > CHUNK || !(TAG_LEN..=CHUNK + TAG_LEN).contains(&enc_n) { let _ = fs::remove_file(&tmp); bail!("invalid container chunk"); }
        let mut enc = vec![0u8; enc_n]; src.read_exact(&mut enc)?; let mut p = [0u8; 2]; src.read_exact(&mut p)?; let pad = u16::from_le_bytes(p) as usize;
        if pad > MAX_PAD { let _ = fs::remove_file(&tmp); bail!("invalid padding"); }
        if pad != 0 { let mut junk = vec![0u8; pad]; src.read_exact(&mut junk)?; }
        let compressed = cipher.decrypt(&nonce(&header.base, index), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad(index, header.plain_len, header.flags) }).map_err(|_| anyhow::anyhow!("authentication failed: wrong password or modified file"))?;
        let plain = zstd::bulk::decompress(&compressed, CHUNK).unwrap_or(compressed);
        if plain.len() != n { let _ = fs::remove_file(&tmp); bail!("invalid decompressed chunk"); }
        digest.update(&plain); dst.write_all(&plain)?; total += n as u64; index += 1;
    }
    if total != header.plain_len { let _ = fs::remove_file(&tmp); bail!("length mismatch: container is damaged"); }
    dst.sync_all()?; drop(dst); fs::rename(tmp, output)?; Ok(())
}

fn inspect(input: &Path) -> Result<()> {
    let mut f = File::open(input)?; let header = read_header(&mut f)?;
...
fn run_jar(input: &Path, java: &str, pass: &[u8], args: &[String]) -> Result<()> {
    if debugger_present() { bail!("debugger detected"); }
    let dir = std::env::temp_dir().join(format!("deobf-{}", std::process::id())); fs::create_dir_all(&dir)?; let jar = dir.join("payload.jar");
    let result = unprotect(input, &jar, pass).and_then(|_| { let mut c = Command::new(java); c.arg("-jar").arg(&jar).args(args); Ok(c.status()?) }); let _ = fs::remove_dir_all(&dir); result.map(|_| ())
}

fn debugger_present() -> bool {
    #[cfg(windows)] { unsafe { windows_sys::Win32::System::Diagnostics::Debug::IsDebuggerPresent() != 0 } }
    #[cfg(target_os = "linux")] { fs::read_to_string("/proc/self/status").map(|s| s.lines().any(|l| l.trim_start().starts_with("TracerPid:") && l.split_whitespace().nth(1).unwrap_or("0") != "0")).unwrap_or(false) }
    #[cfg(not(any(windows, target_os = "linux")))] { false }
}
