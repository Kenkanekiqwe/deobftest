...
fn protect(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    ...
    let tmp = output.with_extension("deobf-write-tmp"); let mut dst = File::create(&tmp)?;
    ...
    let trailer = cipher.encrypt(&nonce(&base, u64::MAX), chacha20poly1305::aead::Payload { msg: final_hash.as_bytes(), aad: &aad(u64::MAX, plain_len, 1) }).map_err(|_| anyhow::anyhow!("trailer encryption failed"))?;
...
fn unprotect(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    if input == output { bail!("input and output must differ"); }
    let mut src = File::open(input)?; let (version, flags, salt, base, plain_len) = read_header(&mut src)?;
    if version == LEGACY_VERSION { bail!("DEOBF v1 reader is not available in this build"); }
    if flags & 1 == 0 { bail!("unsupported DEOBF v2 flags"); }
    let key = derive_key(pass, &salt)?; let cipher = XChaCha20Poly1305::new((&key).into());
    let tmp = output.with_extension("deobf-tmp"); let mut dst = File::create(&tmp)?; let mut total = 0u64; let mut index = 0u64; let mut digest = Hasher::new(); digest.update(b"DEOBF-CONTENT-V2");
    loop {
        let mut marker = [0u8; 4]; src.read_exact(&mut marker)?;
        if &marker == b"TRLR" {
            let mut l = [0u8; 4]; src.read_exact(&mut l)?; let n = u32::from_le_bytes(l) as usize; if n != 48 { let _ = fs::remove_file(&tmp); bail!("invalid trailer"); }
            let mut enc = vec![0u8; n]; src.read_exact(&mut enc)?; let expected = cipher.decrypt(&nonce(&base, u64::MAX), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad(u64::MAX, plain_len, flags) }).map_err(|_| anyhow::anyhow!("trailer authentication failed"))?;
            if expected.len() != 32 || expected.as_slice() != digest.finalize().as_bytes() { let _ = fs::remove_file(&tmp); bail!("content integrity check failed"); } break;
        }
        let n = u32::from_le_bytes(marker) as usize; let mut e = [0u8; 4]; src.read_exact(&mut e)?; let enc_n = u32::from_le_bytes(e) as usize;
        if n == 0 || n > CHUNK || enc_n < TAG_LEN || enc_n > CHUNK + TAG_LEN + 1024 { let _ = fs::remove_file(&tmp); bail!("invalid container chunk"); }
        let mut enc = vec![0u8; enc_n]; src.read_exact(&mut enc)?; let mut p = [0u8; 2]; src.read_exact(&mut p)?; let pad = u16::from_le_bytes(p) as usize; if pad > MAX_PAD { let _ = fs::remove_file(&tmp); bail!("invalid padding"); }
        if pad != 0 { let mut junk = vec![0u8; pad]; src.read_exact(&mut junk)?; }
        let compressed = cipher.decrypt(&nonce(&base, index), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad(index, plain_len, flags) }).map_err(|_| anyhow::anyhow!("authentication failed: wrong password or modified file"))?;
        let plain = zstd::bulk::decompress(&compressed, CHUNK).unwrap_or(compressed); if plain.len() != n { let _ = fs::remove_file(&tmp); bail!("invalid decompressed chunk"); }
...
fn inspect(input: &Path) -> Result<()> { let mut f = File::open(input)?; let (version, flags, _, _, len) = read_header(&mut f)?; println!("format: DEOBF v{}\noriginal size: {} bytes\nchunk size: {} KiB\nflags: 0x{:02x}", version, len, CHUNK / 1024, flags); Ok(()) }
