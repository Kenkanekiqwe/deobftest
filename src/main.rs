...
fn protect(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    ...
    loop {
        ...
        let encrypted = cipher.encrypt(&nonce(&base, index), chacha20poly1305::aead::Payload { msg: &compressed, aad: &aad(index, plain_len, 1) })
    ...
    let trailer = cipher.encrypt(&nonce(&base, u64::MAX), chacha20poly1305::aead::Payload { msg: final_hash.as_bytes(), aad: &aad(u64::MAX, plain_len, 1) })
...
fn unprotect_v1(src: &mut File, output: &Path, pass: &[u8], salt: &[u8; SALT_LEN], base: &[u8; NONCE_LEN], plain_len: u64) -> Result<()> {
    ...
    loop {
        ...
        let plain = cipher.decrypt(&nonce_v1(base, index), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad_v1(index, plain_len) })
...
fn unprotect(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    ...
    loop {
        ...
        if &marker == b"TRLR" {
            ...
            src.read_exact(&mut enc)?;
            let expected = cipher.decrypt(&nonce(&base, u64::MAX), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad(u64::MAX, plain_len, flags) })
        ...
        let compressed = cipher.decrypt(&nonce(&base, index), chacha20poly1305::aead::Payload { msg: &enc, aad: &aad(index, plain_len, flags) })
            .map_err(|_| anyhow::anyhow!("authentication failed: wrong password or modified file"))?;
...
fn inspect(input: &Path) -> Result<()> {
    ...
    println!("format: DEOBF v{}\noriginal size: {} bytes\nchunk size: {} KiB\nflags: 0x{:02x}", version, len, CHUNK / 1024, flags);
    Ok(())
}

fn run_jar(input: &Path, java: &str, pass: &[u8], args: &[String]) -> Result<()> {
    if debugger_present() { bail!("debugger detected"); }
    let dir = std::env::temp_dir().join(format!("deobf-{}", std::process::id()));
    fs::create_dir_all(&dir)?;
    let jar = dir.join("payload.jar");
    let result = unprotect(input, &jar, pass).and_then(|_| {
        let mut c = Command::new(java);
        c.arg("-jar").arg(&jar).args(args);
        Ok(c.status()?)
    });
...
fn debugger_present() -> bool {
    ...
    #[cfg(target_os = "linux")]
    { fs::read_to_string("/proc/self/status").map(|s| s.lines().any(|l| l.trim_start().starts_with("TracerPid:") && l.split_whitespace().nth(1).unwrap_or("0") != "0")).unwrap_or(false) }
