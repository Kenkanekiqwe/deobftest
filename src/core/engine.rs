...
pub fn protect(data: Vec<u8>, options: &EngineOptions) -> Result<(Vec<u8>, EngineResult)> {
    ...
    Ok((output, EngineResult {
        analysis: analysis.into(), input_size, output_size,
        elapsed_ms: started.elapsed().as_millis(), input_hash, output_hash,
        passes, compatibility_mode: false, format_preserved: true,
    }))
}

/// Restore a v2 DEOBF package directly from the library. This is used by the
/// GUI so the password never needs to be passed through a process command line.
pub fn unprotect_file(input: &Path, output: &Path, pass: &[u8]) -> Result<()> {
    ...
    if &magic != MAGIC { bail!("not a DEOBF v2 package"); }
    let mut one = [0u8; 1];
    src.read_exact(&mut one)?;
    if one[0] != VERSION { bail!("unsupported DEOBF version {}", one[0]); }
    let mut flags = [0u8; 1];
    src.read_exact(&mut flags)?;
    if flags[0] & 1 == 0 { bail!("unsupported DEOBF flags"); }
    ...
    let tmp = output.with_extension("deobf-restore-tmp");
    let result = (|| -> Result<()> {
        ...
        loop {
            ...
            if &marker == b"TRLR" {
                ...
                if u32::from_le_bytes(l) as usize != 48 { bail!("invalid trailer"); }
                let mut enc = vec![0u8; 48];
                src.read_exact(&mut enc)?;
                let expected = cipher.decrypt(
                    &nonce(&base, u64::MAX),
                    Payload { msg: &enc, aad: &aad(u64::MAX, plain_len, flags[0]) },
                ).map_err(|_| anyhow::anyhow!("authentication failed: wrong password or modified package"))?;
                if expected.as_slice() != digest.finalize().as_bytes() { bail!("content integrity check failed"); }
                break;
            }
            ...
            let compressed = cipher.decrypt(
                &nonce(&base, index),
                Payload { msg: &enc, aad: &aad(index, plain_len, flags[0]) },
            ).map_err(|_| anyhow::anyhow!("authentication failed: wrong password or modified package"))?;
            let plain = zstd::bulk::decompress(&compressed, CHUNK).unwrap_or(compressed);
            if plain.len() != n { bail!("invalid decompressed chunk"); }
            digest.update(&plain);
            dst.write_all(&plain)?;
            total += n as u64;
            index += 1;
