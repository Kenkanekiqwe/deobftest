...
use super::{
    artifact::{detect, ArtifactKind},
    engine::unprotect_file,
};
...
pub fn launch_restored(
    ...
    if matches!(kind, ArtifactKind::Jar | ArtifactKind::Pe) {
        command.args(args);
    }
    command
        .status()
        .context("launch restored protected payload")
}
...
pub fn launch_python(payload: &Path, policy: &LoaderPolicy, args: &[String]) -> Result<ExitStatus> {
    ...
    if let Some(dir) = policy.working_directory.as_deref() {
        command.current_dir(dir);
    }
...
pub fn detect_restored(payload: &Path) -> Result<ArtifactKind> {
    let bytes = std::fs::read(payload)
        .with_context(|| format!("read restored payload {}", payload.display()))?;
    Ok(detect(&bytes))
}
