...
fn get_string(data: &[u8], cursor: &mut usize) -> Result<String> {
    if data.len().saturating_sub(*cursor) < 4 { bail!("truncated compatibility metadata"); }
    let mut len = [0u8; 4];
    len.copy_from_slice(&data[*cursor..*cursor + 4]);
    *cursor += 4;
    let len = u32::from_le_bytes(len) as usize;
    if data.len().saturating_sub(*cursor) < len { bail!("truncated compatibility metadata"); }
...
fn decode_manifest(data: &[u8]) -> Result<CompatibilityManifest> {
    if data.len() < 12 { bail!("invalid compatibility metadata payload"); }
    ...
    if cursor != data.len() { bail!("unexpected data after compatibility metadata"); }
    Ok(CompatibilityManifest { version: u32::from_le_bytes(version), artifact, original_size: u64::from_le_bytes(size), original_hash, profile })
}

/// Compatibility mode preserves artifact bytes exactly and stores authenticated
/// protection metadata beside the output. This keeps EXE/DLL/JAR/PY/ZIP and
...
pub fn protect_compatible(input: &Path, output: &Path, password: &[u8], artifact: &str, profile: &str) -> Result<PathBuf> {
    ...
    let manifest = CompatibilityManifest { version: 1, artifact: artifact.to_owned(), original_size: data.len() as u64, original_hash: hash(&data), profile: profile.to_owned() };
    ...
    let encrypted = cipher.encrypt(&XNonce::from(nonce), Payload { msg: &plain, aad: MAGIC })
...
pub fn verify_compatible(input: &Path, password: &[u8]) -> Result<CompatibilityManifest> {
    ...
    if meta.len() < MAGIC.len() + SALT_LEN + NONCE_LEN + 16 || &meta[..MAGIC.len()] != MAGIC { bail!("invalid compatibility metadata"); }
    let mut salt = [0u8; SALT_LEN];
    ...
    let key = derive_key(password, &salt)?;
    let cipher = XChaCha20Poly1305::new((&key).into());
    let plain = cipher.decrypt(&XNonce::from(nonce), Payload { msg: encrypted, aad: MAGIC })
        .map_err(|_| anyhow::anyhow!("metadata authentication failed"))?;
    let manifest = decode_manifest(&plain)?;
    if manifest.original_size != data.len() as u64 || manifest.original_hash != hash(&data) { bail!("protected file integrity check failed"); }
    Ok(manifest)
}
